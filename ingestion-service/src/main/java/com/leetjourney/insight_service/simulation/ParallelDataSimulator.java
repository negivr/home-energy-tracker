package com.leetjourney.insight_service.simulation;

import com.leetjourney.insight_service.dto.EnergyUsageDto;
        ...
        for (int i = 0; i < parallelThreads; i++) {
            ...
            executorService.submit(() -> {
                for (int j = 0; j < requestsForThread; j++) {
                    EnergyUsageDto dto = EnergyUsageDto.builder().deviceId(random.nextLong(1, 6)).energyConsumed(Math.round(random.nextDouble(0.0, 2.0) * 100.0) / 100.0).timestamp(LocalDateTime.now().atZone(ZoneId.systemDefault()).toInstant()).build();
                    try {
